...

{
    ...
    public KeyCode zoomKey = KeyCode.Z; // Hoặc bất kỳ phím nào khác bạn không dùng
    ...
    public Vector3 bobAmount = new Vector3(.15f, .01f, 0f);
    ...
    private void Update()
    {
        ...
        // if (cameraCanMove)
        // {
        //     //  Kiểm tra xem đang nhấn chuột phải vào object có tag "Table"
        //     //Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
        //     Ray ray = playerCamera.ScreenPointToRay(Input.mousePosition);
        //     RaycastHit hit;
        //     bool isLookingAtTable = Input.GetMouseButton(1) &&
        //                             Physics.Raycast(ray, out hit, 3f) &&
        //                             hit.collider.CompareTag("keepZ");
        ...
        {
            allowLook = Input.GetMouseButton(0) ; // Chỉ cho phép xoay khi giữ chuột trái ở chế độ View
        }
        // if (Cursor.visible) return;
        if (cameraCanMove && allowLook)
        {
            // Nếu muốn chặn xoay khi đang nhấn vào object nào đó, bạn có thể bổ sung kiểm tra raycast ở đây nếu cần
            if (ViewPlayer.isViewMode)
            {
                yaw = transform.localEulerAngles.y - Input.GetAxis("Mouse X") * mouseSensitivity;
            }
    ...
    void FixedUpdate()
    {
        float minX = -3.5f, maxX = 5f;   // Thay bằng giá trị phù hợp với phòng của bạn
        float minZ = -3.5f, maxZ = 3.5f;

        Vector3 rbPos = rb.position;
        rbPos.x = Mathf.Clamp(rbPos.x, minX, maxX);
        rbPos.z = Mathf.Clamp(rbPos.z, minZ, maxZ);
        rb.position = rbPos;
        #region Movement
    ...
    private void HeadBob()
    {
        ...
        //Debug.Log($"{joint.localPosition.x}, {joint.localPosition.y}, {joint.localPosition.z}"); // Debugging line to check joint position
    }
}
